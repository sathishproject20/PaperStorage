...
class AppData:
    ...
    def read_json_data(cls, json_file):
        try:
            ...
            cls.asciiColorIndex = data['asciiColorIndex']

            # Logging to check initialized attributes
            logging.info(f"asciiColorIndex: {cls.asciiColorIndex}")
...
AppData.read_json_data('ASCIIData.json')
...
class AsciiPatternGenerator:
    def __init__(self, app_template, app_data):
        ...
        self.asciiLargeString = ""

    def set_ascii_large_string(self, ascii_large_string):
        # Limit to 4096 characters
        if len(ascii_large_string) > 512:
            messagebox.showerror("Error", "Please enter an ASCII string with up to 4096 characters.")
            return

        self.asciiLargeString = ascii_large_string.upper()

    def is_valid_ascii(self, ascii_string):
        try:
        ...
        for y in range(sqrt_chars):
            for x in range(sqrt_chars):
                ...
                if index < num_chars:
                    ...
                    color = self.app_data.asciiColorIndex.get(str(decimal_value), '#000000')
                    self.draw_pixel(canvas, x * pixel_size, y * pixel_size, pixel_size, color)

    def start_drawing(self, canvas, max_width, max_height):
        self.draw_image_canvas(canvas, self.asciiLargeString, max_width, max_height)

class AppTemplate:
    def __init__(self, root):
        self.root = root
        self.app_data = AppData()
        self.pattern_generator = AsciiPatternGenerator(self, self.app_data)
        self.root.title("ASCII Large Pattern Generator")
        ...
        self.ascii_input = Text(self.main_view, width=100, height=10)
        self.ascii_input.pack(pady=20, padx=20, side=tk.TOP, fill=tk.X)
    ...
    def generate_pattern(self):
        ascii_large_string = self.ascii_input.get("1.0", tk.END).strip()
        if self.pattern_generator.is_valid_ascii(ascii_large_string):
            self.pattern_generator.set_ascii_large_string(ascii_large_string)
            self.pattern_generator.start_drawing(self.image_canvas, max_width, max_height)
            self.update_scroll_region()
        else:
            messagebox.showerror("Error", "Invalid ASCII Input. Please enter a valid ASCII string.")
            ...
            try:
                ...
                for y in range(sqrt_chars):
                    for x in range(sqrt_chars):
                        index = y * sqrt_chars + x
                        if index < num_chars:
                            char = self.pattern_generator.asciiLargeString[index]
                            decimal_value = ord(char)
                            color = self.app_data.asciiColorIndex.get(str(decimal_value), '#000000')
...
def main():
    ...
    root.title("ASCII Large Pattern Generator")
