...
def create_json_from_text_files(keys_folder, values_folder, output_folder='ascii4pattern_files'):
    # Ensure the output folder exists
    os.makedirs(output_folder, exist_ok=True)
    
    # Get sorted list of files from both folders
    keys_files = sorted([f for f in os.listdir(keys_folder) if f.startswith('ASCII4Permutations_') and f.endswith('.txt')])
    ...
    for key_file, value_file in zip(keys_files, values_files):
        ...
        output_file = os.path.join(output_folder, f'ASCII4Pattern_{key_file.split("_")[1].split(".")[0]}.json')
