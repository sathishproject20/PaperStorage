import tkinter as tk
from tkinter import Canvas, filedialog, messagebox
import os
from PIL import Image, ImageDraw, ImageTk

hex12String = []

# Function to validate hexadecimal input
def validateHex(P):
    if P == "":
        return True
    if all(c in "0123456789abcdefABCDEF" for c in P) and len(P) <= 12:
        return True
    return False

class PatternGenerator:
    def __init__(self, root):
        self.root = root

        # Define color mappings for HexOne and HexTwo
        self.hexOneColorIndex = {
            "0": "#7CB9E8","1": "#00FA9A", "2": "#87CEFA", "3": "#F0F8FF",
            "4": "#00FFFF","5": "#7FFFD4","6": "#0000CD", "7": "#9370DB",
            "8": "#90EE90","9": "#ADFF2F", "A": "#FF8C00", "B": "#008000",
            "C": "#0000ff","D": "#ff0000", "E": "#00FFFF", "F": "#00ff40"
        }

        self.hexTwoColorIndex = {
            "0": "#cc0000","1": "#008000", "2": "#000FFF", "3": "#00008B",
            "4": "#cc00cc","5": "#9900cc","6": "#6600cc", "7": "#3300cc",
            "8": "#A52A2A","9": "#006400", "A": "#940003", "B": "#ADFF2F",
            "C": "#800000","D": "#cc6600", "E": "#FFA500", "F": "#FF4500"
        }

        # Hex pixel positions
        self.hexOnePxPosition = {
            (0, 2): '0', (0, 3): '0', (0, 4): '0', (0, 5): '0', (0, 6): '1', (0, 7): '1',
            (0, 8): '1', (0, 9): '1', (0, 10): '2', (0, 11): '2', (0, 12): '2', (0, 13): '2',
            (1, 2): '0', (1, 3): '0', (1, 4): '0', (1, 5): '0', (1, 6): '1', (1, 7): '1',
            (1, 8): '1', (1, 9): '1', (1, 10): '2', (1, 11): '2', (1, 12): '2', (1, 13): '2',
            (2, 2): '0', (2, 3): '0', (2, 4): '0', (2, 5): '0', (2, 6): '1', (2, 7): '1',
            (2, 8): '1', (2, 9): '1', (2, 10): '2', (2, 11): '2', (2, 12): '2', (2, 13): '2',
            (3, 2): '0', (3, 3): '0', (3, 4): '0', (3, 5): '0', (3, 6): '1', (3, 7): '1',
            (3, 8): '1', (3, 9): '1', (3, 10): '2', (3, 11): '2', (3, 12): '2', (3, 13): '2',
            (4, 2): '3', (4, 3): '3', (4, 4): '3', (4, 5): '3', (4, 6): '4', (4, 7): '4',
            (4, 8): '4', (4, 9): '4', (4, 10): '5', (4, 11): '5', (4, 12): '5', (4, 13): '5',
            (5, 2): '3', (5, 3): '3', (5, 4): '3', (5, 5): '3', (5, 6): '4', (5, 7): '4',
            (5, 8): '4', (5, 9): '4', (5, 10): '5', (5, 11): '5', (5, 12): '5', (5, 13): '5',
            (6, 2): '3', (6, 3): '3', (6, 4): '3', (6, 5): '3', (6, 6): '4', (6, 7): '4',
            (6, 8): '4', (6, 9): '4', (6, 10): '5', (6, 11): '5', (6, 12): '5', (6, 13): '5',
            (7, 2): '3', (7, 3): '3', (7, 4): '3', (7, 5): '3', (7, 6): '4', (7, 7): '4',
            (7, 8): '4', (7, 9): '4', (7, 10): '5', (7, 11): '5', (7, 12): '5', (7, 13): '5',
        }

        self.hexTwoPxPosition = {
            (8, 2): '0', (8, 3): '0', (8, 4): '0', (8, 5): '0', (8, 6): '1', (8, 7): '1',
            (8, 8): '1', (8, 9): '1', (8, 10): '2', (8, 11): '2', (8, 12): '2', (8, 13): '2',
            (9, 2): '0', (9, 3): '0', (9, 4): '0', (9, 5): '0', (9, 6): '1', (9, 7): '1',
            (9, 8): '1', (9, 9): '1', (9, 10): '2', (9, 11): '2', (9, 12): '2', (9, 13): '2',
            (10, 2): '0', (10, 3): '0', (10, 4): '0', (10, 5): '0', (10, 6): '1', (10, 7): '1',
            (10, 8): '1', (10, 9): '1', (10, 10): '2', (10, 11): '2', (10, 12): '2', (10, 13): '2',
            (11, 2): '0', (11, 3): '0', (11, 4): '0', (11, 5): '0', (11, 6): '1', (11, 7): '1',
            (11, 8): '1', (11, 9): '1', (11, 10): '2', (11, 11): '2', (11, 12): '2', (11, 13): '2',
            (12, 2): '3', (12, 3): '3', (12, 4): '3', (12, 5): '3', (12, 6): '4', (12, 7): '4',
            (12, 8): '4', (12, 9): '4', (12, 10): '5', (12, 11): '5', (12, 12): '5', (12, 13): '5',
            (13, 2): '3', (13, 3): '3', (13, 4): '3', (13, 5): '3', (13, 6): '4', (13, 7): '4',
            (13, 8): '4', (13, 9): '4', (13, 10): '5', (13, 11): '5', (13, 12): '5', (13, 13): '5',
            (14, 2): '3', (14, 3): '3', (14, 4): '3', (14, 5): '3', (14, 6): '4', (14, 7): '4',
            (14, 8): '4', (14, 9): '4', (14, 10): '5', (14, 11): '5', (14, 12): '5', (14, 13): '5',
            (15, 2): '3', (15, 3): '3', (15, 4): '3', (15, 5): '3', (15, 6): '4', (15, 7): '4',
            (15, 8): '4', (15, 9): '4', (15, 10): '5', (15, 11): '5', (15, 12): '5', (15, 13): '5',
        }

         # Header
        self.header = tk.Frame(root, bg='#0000FF', height=50)
        self.header.pack(side='top', fill='x')
        self.header_label = tk.Label(self.header, text="Hex 12 Pattern Generator", bg='#0000FF', fg='White', font=('Arial', 20, 'bold'))
        self.header_label.pack(pady=10)

        # Side View
        self.side_view = tk.Frame(root, bg='lightgrey', width=200, height=600)
        self.side_view.pack(side='left', fill='y')
        self.side_view.pack_propagate(False)

        # Main View
        self.main_view = tk.Frame(root, bg='white', width=600, height=600)
        self.main_view.pack(side='right', expand=True, fill='both')


        # Canvas Widget
        self.imageCanvas = Canvas(self.main_view, width=256, height=256, bg='white')
        self.imageCanvas.pack(pady=20)

        # Label and Entry for Hex Input
        self.hexLabel = tk.Label(self.side_view, text="Enter Hex String:")
        self.hexLabel.pack(pady=5)

        vcmd = (root.register(validateHex), '%P')
        self.hexInput = tk.Entry(self.side_view, validate='key', validatecommand=vcmd)
        self.hexInput.pack(pady=5)

        # Generate Button
        self.generateButton = tk.Button(self.side_view, text="Generate Image", command=self.drawPattern)
        self.generateButton.pack(pady=5)


    def drawPattern(self):
        self.imageCanvas.delete("all")  # Clear the canvas before drawing

        hex12String = self.hexInput.get().upper()

        if validateHex(hex12String):
            imageCanvas = Image.new('RGB', (256, 256), 'white')
            draw = ImageDraw.Draw(imageCanvas)

            # HexOne: First six characters of Hex12String.
            hexOne = hex12String[:6]

            # HexTwo: Last six characters of Hex12String.
            hexTwo = hex12String[6:]

            # Draw HexOne pattern
            for key, hexValue in self.hexOnePxPosition.items():
                row, col = key
                colorIndex = int(hexValue)
                hexOneSequence = hexOne[colorIndex]
                color = self.hexOneColorIndex.get(hexOneSequence, '#FFFFFF')
                draw.rectangle([col * 16, row * 16, (col + 1) * 16, (row + 1) * 16], fill=color)

            # Draw HexTwo pattern
            for key, hexValue in self.hexTwoPxPosition.items():
                row, col = key
                colorIndex = int(hexValue)
                hexTwoSequence = hexTwo[colorIndex]
                color = self.hexTwoColorIndex.get(hexTwoSequence, '#FFFFFF')
                draw.rectangle([col * 16, row * 16, (col + 1) * 16, (row + 1) * 16], fill=color)

            self.image = imageCanvas
            self.displayImage(imageCanvas)
        else:
            messagebox.showerror("Error", "Invalid Hex Input. Please enter a valid hexadecimal string (up to 12 characters).")

    def displayImage(self, image):
        self.tkImage = ImageTk.PhotoImage(image)
        self.imageCanvas.create_image(0, 0, anchor=tk.NW, image=self.tkImage)

# Initialize Tkinter
root = tk.Tk()
root.title("Hex 12 Pattern Generator")
root.geometry("800x800")



# Create PatternGenerator instance
app = PatternGenerator(root)

# Start Tkinter event loop
root.mainloop()
