...
def create_json_from_text_files(keys_folder, values_folder, output_folder='output_json'):
    # Ensure the output folder exists
    os.makedirs(output_folder, exist_ok=True)
    
    # Get sorted list of files from both folders
    keys_files = sorted([f for f in os.listdir(keys_folder) if f.startswith('Hex8Permutations_') and f.endswith('.txt')])
    values_files = sorted([f for f in os.listdir(values_folder) if f.startswith('EncryptionKeys_') and f.endswith('.txt')])
    ...
    for key_file, value_file in zip(keys_files, values_files):
        ...
        output_file = os.path.join(output_folder, f'Hex8EncryptionKeys_{key_file.split("_")[1].split(".")[0]}.json')
...
# Example usage:
keys_folder = 'Hex8Permutations_1'
values_folder = 'EncryptionKeys_1'
