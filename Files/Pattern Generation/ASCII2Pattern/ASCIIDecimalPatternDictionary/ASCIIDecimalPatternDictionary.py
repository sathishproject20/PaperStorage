...
def create_json_from_text_files(keys_folder, values_folder, output_folder='output_json'):
    ...
    keys_files = sorted([f for f in os.listdir(keys_folder) if f.startswith('ASCII2Permutations') and f.endswith('.txt')])
    values_files = sorted([f for f in os.listdir(values_folder) if f.startswith('16Bit_BitMap') and f.endswith('.txt')])
    ...
    for key_file, value_file in zip(keys_files, values_files):
        ...
        output_file = os.path.join(output_folder, f'ASCIIPattern_BitMap.json')
